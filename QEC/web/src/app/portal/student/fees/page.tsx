...
const fees: FeeRecord[] = [
  { id: "1", title: "Tuition Fee (July)", type: "Tuition", amount: 4000, dueDate: "Jul 15, 2026", status: "PAID", paidDate: "Jul 10, 2026", paidAmount: 4000 },
  { id: "2", title: "Lab Fee (July)", type: "Lab", amount: 500, dueDate: "Jul 15, 2026", status: "PAID", paidDate: "Jul 10, 2026", paidAmount: 500 },
...
export default function StudentFeesPage() {
  ...
  const filtered = statusFilter === "all" ? fees : fees.filter((f) => f.status === statusFilter.toUpperCase())
  const totalPaid = fees.filter((f) => f.status === "PAID").reduce((s, f) => s + (f.paidAmount ?? f.amount), 0)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-1">View and manage your fee payments.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-xl border border-border bg-card">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center"><AlertCircle className="h-5 w-5 text-primary" /></div>
            <div><p className="text-2xl font-bold text-foreground">{fees.filter((f) => f.status === "PENDING" || f.status === "OVERDUE").length}</p><p className="text-xs text-muted">Due Items</p></div>
          </div>
        </div>
      </div>
        ...
        {["all", "paid", "pending", "overdue"].map((s) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors ${
              statusFilter === s ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted hover:text-foreground"
            }`}
          >
            {s}
          </button>
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="text-right">
                ...
                <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full mt-1 ${
                  fee.status === "PAID" ? "bg-accent/10 text-accent" :
